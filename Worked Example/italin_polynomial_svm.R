...
p <- ggplot() +geom_line(data=historical_plot_data, 
                         aes(x=Year, y=MSW, color="Observed"), linewidth=0.5)+
  geom_point(data=historical_plot_data,  aes(x=Year, y=MSW, color="Observed"), 
             size=1.5)+ 
  ...
  scale_color_manual(name=NULL, 
                     values=c("Observed"="black","Naive Forecast"="firebrick"))+
  scale_fill_manual(name=NULL, values=c("Simulated MSW Range" = "steelblue"))+
